...
{ 
  ...
  #pragma omp parallel reduction(+: longMedia) firstprivate(min, max, longHilo, its) private(longitud)
  {
    hilosUsados = omp_get_num_threads();

    int i, j, d, im, dm;// dm es la distancia del código más parecido, im su índice

    #pragma omp for private(i, d, dm, im)
    for ( j = 0 ; j < nt2 ; j++ ) {

      dm = MAX_LON + 1;
      longitud = strlen(tabla2[j]);
    ...
    longMedia = longHilo / (float) its;
    
    printf("Hilo %i de %i: Longitud min/med/max: %i/%.2f/%i\n", omp_get_thread_num(), omp_get_num_threads(),
	min, longMedia, max);
  ...
  printf("TOTAL: Longitud min/med/max: %i/%.2f/%i\n", minLongTotal, longMedia / hilosUsados, maxLongTotal);
...
{ 
  ...
  if ( err ) err = 2;
    ...
    if ( ps == NULL ) {
      ...
      printf("El tiempo de ejecución de busqueda de parecidos es %.3f\n", tf - ti);
